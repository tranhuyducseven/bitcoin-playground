...
func TestSendTxWithP2WPKH(t *testing.T) {
	...
	var choosenUtxo *btcjson.ListUnspentResult

	utxos, err := fromWallet.ListUnspent(1, 9999999, fromName)
	assert.Nil(t, err)

	for _, utxo := range utxos {
		if hash160PubKey == utxo.ScriptPubKey[4:] {
			choosenUtxo = utxo
			t.Log("Found utxo: ", choosenUtxo)
			break
		}
	}

	if choosenUtxo == nil {
		t.Log("Not found utxo -> Fund the new one")
